...
public class No_787_CheapestFlightsWithinKStops{
    public static void main(String[] args) {
       Solution solution = new No_787_CheapestFlightsWithinKStops().new Solution();
...
class Solution {
    ...
    public int findCheapestPrice1(int n, int[][] flights, int src, int dst, int k) {
        ...
        boolean[] walked = new boolean[n];
        helper(src,walked,graph,dst,k,0);

        return flag?res:-1;
    }

    public void helper(int st, boolean[] walked, int[][] graph, int dst, int k, int cost){
        if (st == dst){
            flag = true;
        ...
        for (int i = 0; i < walked.length; i++){
            ...
            if (graph[st][i] == 0 || walked[i]){
                continue;
            }
            helper(i,walked,graph,dst,k-1,cost+graph[st][i]);
        }
